...
function render(){
if(!S.user){let h='<div class="login"><div class="login-logo">Tas<span class="k">k</span>it</div>';
...
// Tabs
h+='<div class="tabs"><button class="tab'+(S.tab==='tasks'?' on':'')+'" onclick="switchTab(\\'tasks\\')">\\u{1F4CB} Tasks</button><button class="tab'+(S.tab==='books'?' on':'')+'" onclick="switchTab(\\'books\\')">\\u{1F4DA} Books</button><button class="tab'+(S.tab==='guide'?' on':'')+'" onclick="switchTab(\\'guide\\')">\\u{1F680} Go Live</button></div>';
...
// GUIDE TAB
  ...
  h+='<h2 style="font-family:\\'Space Mono\\',monospace;font-size:18px;margin-bottom:8px">\\u{1F680} Publish Taskit to Play Store</h2>';
  h+='<p style="font-size:13px;color:#6B665E;margin-bottom:12px">Your web app \\u2192 Android app via Trusted Web Activity (TWA).</p>';
  h+='<div class="callout"><b>Cost:</b> $25 one-time Google Play Developer fee. No recurring charges.</div>';
  h+='<div class="guide-card"><h3><span class="num">1</span>Google Play Developer account</h3><p>Visit <code>play.google.com/console/signup</code>. Pay $25. Verify identity — takes <b>1–7 days</b>.</p><div class="callout"><b>That\\'s your "verification pending".</b> Nothing to do but wait for email.</div></div>';
  h+='<div class="guide-card"><h3><span class="num">2</span>Custom domain</h3><p>Play Store needs verified HTTPS. Replace <code>taskit.up.railway.app</code> with:</p><ul><li><b>taskit.app</b> — ~$20/yr</li><li><b>taskit.live</b> — ~$15/yr</li><li><b>gettaskit.com</b> — ~$12/yr</li></ul><p>Buy at namecheap.com, point DNS to Railway.</p></div>';
  h+='<div class="guide-card"><h3><span class="num">3</span>Generate Android APK</h3><p>Run in terminal:</p><ul><li><code>npm i -g @bubblewrap/cli</code></li><li><code>bubblewrap init --manifest=https://taskit.app/manifest.json</code></li><li><code>bubblewrap build</code></li></ul><p>Output: <code>app-release-signed.aab</code></p></div>';
  h+='<div class="guide-card"><h3><span class="num">4</span>Play Store listing</h3><ul><li>Icon 512×512 PNG</li><li>Feature graphic 1024×500</li><li>2–8 screenshots</li><li>Short + long description</li><li>Privacy policy URL</li><li>Content rating</li></ul></div>';
  h+='<div class="guide-card"><h3><span class="num">5</span>Submit & wait</h3><p>Upload .aab, fill listing, submit. First review: <b>2–7 days</b>. Google emails when live.</p></div>';
}
