...
async function playBook(id,title,author){S.playing={id,title,author,loading:true};render();try{const r=await fetch('https://archive.org/metadata/'+id);const j=await r.json();if(!j.files){toast('\\u26A0\\uFE0F No files','err');S.playing=null;render();return}let mp3=j.files.find(f=>/_64kb\\.mp3$/i.test(f.name));if(!mp3)mp3=j.files.find(f=>/\\.mp3$/i.test(f.name)&&!/sample|test|spoken/i.test(f.name));if(mp3){const url='https://archive.org/download/'+id+'/'+mp3.name.split('/').map(encodeURIComponent).join('/');S.playing={id,title,author,url};render();setTimeout(()=>{const a=document.getElementById('audioEl');if(a){a.src=url;a.load();a.play().catch(err=>toast('\\u25B6\\uFE0F Tap play button','err'))}},150)}else{toast('\\u26A0\\uFE0F No MP3 in this book','err');S.playing=null;render()}}catch(e){toast('\\u26A0\\uFE0F '+e.message,'err');S.playing=null;render()}}
...
if(S.showProfile){
  ...
  h+='<h2 style="margin-bottom:4px">'+esc(S.user.name||'Taskit User')+'</h2>';
