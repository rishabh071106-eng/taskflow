...
async function sendWA(to,body){
  ...
  try{const n=to.startsWith('whatsapp:')?to:'whatsapp:'+to;const m=await tw.messages.create({from:TW_FROM,to:n,body});return{ok:true,sid:m.sid}}catch(e){return{ok:false,reason:e.message}}
}
...
app.post('/api/login',async(req,res)=>{
  let{phone,name}=req.body;phone=cleanPhone(phone);
  if(!phone||phone.length<10)return res.status(400).json({error:'Valid phone number required'});
  if(!phone.startsWith('+'))phone='+'+phone;
  let user=db.prepare('SELECT * FROM users WHERE phone=?').get(phone);
  if(!user){const token=genToken();db.prepare('INSERT INTO users(phone,name,token)VALUES(?,?,?)').run(phone,name||'',token);user={phone,name:name||'',token}}
  else if(!user.token){const token=genToken();db.prepare('UPDATE users SET token=?,name=COALESCE(NULLIF(?,\'\'),name) WHERE phone=?').run(token,name||'',phone);user.token=token}
  else if(name){db.prepare('UPDATE users SET name=? WHERE phone=?').run(name,phone);user.name=name}
  res.json({phone:user.phone,name:user.name,token:user.token});
});
...
app.post('/api/tasks',auth,async(req,res)=>{
  ...
  const id=genId();
  db.prepare('INSERT INTO tasks(id,user_phone,title,notes,priority,status,due_date,reminder_time,source)VALUES(?,?,?,?,?,?,?,?,?)').run(id,req.user.phone,title.trim(),notes||'',priority||'medium',status||'pending',due_date||'',reminder_time||'','app');
...
app.post('/api/send-task/:id',auth,async(req,res)=>{
  const t=db.prepare('SELECT * FROM tasks WHERE id=? AND user_phone=?').get(req.params.id,req.user.phone);
  if(!t)return res.status(404).json({error:'Not found'});
  const p=PRI[t.priority]||'🟠';let msg=`📋 *Task Reminder*\n\n${p} *${t.title}*`;
  if(t.notes)msg+=`\n${t.notes}`;if(t.due_date)msg+=`\n📅 Due: ${fmtD(t.due_date)}`;
  msg+=`\n\n_Reply "done ${t.title.slice(0,20)}" to complete_`;
  res.json(await sendWA(req.user.phone,msg));
});

app.post('/api/send-all',auth,async(req,res)=>{
  const tasks=db.prepare("SELECT * FROM tasks WHERE user_phone=? AND status!='done' ORDER BY priority DESC").all(req.user.phone);
...
function parseIncoming(text){
  const r={title:text.trim(),priority:'medium',dueDate:'',command:null};const low=text.toLowerCase().trim();
  ...
  if(/\b(urgent|important|asap|critical|high priority)\b/i.test(low)){r.priority='high';r.title=r.title.replace(/\b(urgent|important|asap|critical|high priority)\b/gi,'')}
  else if(/\b(low priority|no rush|whenever)\b/i.test(low)){r.priority='low';r.title=r.title.replace(/\b(low priority|no rush|whenever)\b/gi,'')}
...
app.post('/api/webhook/whatsapp',async(req,res)=>{
  ...
  console.log(`📨 WA ${phone}: ${body}`);
  // Auto-create user if new
  let user=db.prepare('SELECT * FROM users WHERE phone=?').get(phone);
  if(!user){const token=genToken();db.prepare('INSERT INTO users(phone,token)VALUES(?,?)').run(phone,token);user={phone,token}}
  const parsed=parseIncoming(body);
  const twiml=new twilio.twiml.MessagingResponse();

  if(parsed.command==='help'){
    twiml.message(`🤖 *TaskFlow Commands*\n\n📝 Just type a task to add it\n   Ex: "Buy groceries tomorrow urgent"\n\n📋 "list" — see your tasks\n✅ "done buy groceries" — mark done\n🔄 "doing fix bug" — start task\n🗑️ "delete buy groceries" — remove\n\n🌐 Open your tasks: https://taskit.up.railway.app\n\n_Priority: "urgent" or "low priority"_\n_Dates: "today", "tomorrow", "next week"_`);
  } else if(parsed.command==='list'){
    const tasks=db.prepare("SELECT * FROM tasks WHERE user_phone=? AND status!='done' ORDER BY priority DESC").all(phone);
    if(!tasks.length)twiml.message("✨ No pending tasks!");
    else{let m=`📋 *Your Tasks (${tasks.length})*\n`;tasks.forEach((t,i)=>{m+=`\n${i+1}. ${PRI[t.priority]} ${t.title}${t.status==='in-progress'?' 🔄':''}${t.due_date?' _('+fmtD(t.due_date)+')_':''}`});twiml.message(m)}
  } else if(parsed.command==='done'){
    const t=db.prepare("SELECT * FROM tasks WHERE user_phone=? AND status!='done' AND LOWER(title) LIKE ? ORDER BY created_at DESC").get(phone,`%${parsed.title}%`);
    ...
    const t=db.prepare("SELECT * FROM tasks WHERE user_phone=? AND status='pending' AND LOWER(title) LIKE ?").get(phone,`%${parsed.title}%`);
    ...
    const t=db.prepare("SELECT * FROM tasks WHERE user_phone=? AND LOWER(title) LIKE ?").get(phone,`%${parsed.title}%`);
    if(t){db.prepare('DELETE FROM tasks WHERE id=?').run(t.id);twiml.message(`🗑️ Deleted: *${t.title}*`)}
    else twiml.message(`❌ No task matching "${parsed.title}"`)
  } else if(parsed.title){
    const id=genId();db.prepare("INSERT INTO tasks(id,user_phone,title,priority,due_date,source)VALUES(?,?,?,?,?,'whatsapp')").run(id,phone,parsed.title,parsed.priority,parsed.dueDate);
    let m=`✅ *Added!*\n\n${PRI[parsed.priority]} ${parsed.title}`;if(parsed.dueDate)m+=`\n📅 Due: ${fmtD(parsed.dueDate)}`;
    m+=`\n\n🌐 View all: https://taskit.up.railway.app`;twiml.message(m);
...
.st b{font-family:'Space Mono',monospace;font-size:20px;display:block}.st small{font-size:9px;color:#9C968D;text-transform:uppercase;letter-spacing:.5px;font-weight:600}
...
.chk{width:22px;height:22px;min-width:22px;border-radius:6px;border:2px solid #DDD8D1;display:flex;align-items:center;justify-content:center;flex-shrink:0;background:transparent}.chk.on{background:#3DAE5C;border-color:#3DAE5C}
...
.login{max-width:400px;margin:0 auto;padding:60px 24px;text-align:center}
.login-logo{font-family:'Space Mono',monospace;font-size:32px;font-weight:700;margin-bottom:8px}
.login-sub{font-size:14px;color:#6B665E;margin-bottom:32px;line-height:1.5}
.login input{margin-bottom:12px;text-align:center;font-size:18px;letter-spacing:1px;padding:14px}
.login .mb-s{width:100%;padding:14px;font-size:16px;border-radius:12px;margin-top:4px}
...
<script>
let S={tasks:[],view:'all',search:'',showAdd:false,editing:null,listening:false,vt:'',toast:null,waOk:false,sending:{},user:null,
form:{title:'',notes:'',priority:'medium',dueDate:'',reminderTime:'',status:'pending'},
loginForm:{phone:'',name:''}};
let rec=null,token=localStorage.getItem('tf_token'),userPhone=localStorage.getItem('tf_phone'),userName=localStorage.getItem('tf_name');
if(token)S.user={phone:userPhone,name:userName,token};
...
async function login(){
  let ph=S.loginForm.phone.replace(/[^0-9+]/g,'');if(!ph.startsWith('+'))ph='+'+ph;
  if(ph.length<10){toast('\\u26A0\\uFE0F Enter valid phone with country code');return}
  const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:ph,name:S.loginForm.name})}).then(r=>r.json());
  if(r.token){token=r.token;localStorage.setItem('tf_token',r.token);localStorage.setItem('tf_phone',r.phone);localStorage.setItem('tf_name',r.name);S.user=r;load();chk();toast('\\u2705 Welcome!')}
  else toast('\\u26A0\\uFE0F Login failed');
}
function logout(){token=null;S.user=null;S.tasks=[];localStorage.removeItem('tf_token');localStorage.removeItem('tf_phone');localStorage.removeItem('tf_name');render()}
...
function render(){
if(!S.user){
  document.getElementById('app').innerHTML='<div class="login"><div class="login-logo">TaskFlow</div><div class="login-sub">Your tasks, your WhatsApp, one place.<br>Log in with your phone number.</div><input type="text" placeholder="Your name" value="'+esc(S.loginForm.name)+'" oninput="S.loginForm.name=this.value"><input type="tel" placeholder="+91 98765 43210" value="'+esc(S.loginForm.phone)+'" oninput="S.loginForm.phone=this.value" style="font-family:\\'Space Mono\\',monospace"><button class="mb mb-s" onclick="login()">Get Started</button><div class="login-hint">Use the same number as your WhatsApp.<br>No password needed \\u2014 your phone number is your account.</div></div>'+(S.toast?'<div class="toast">'+S.toast+'</div>':'');
  return;
}
const ts=S.tasks;
...
if(S.waOk)h+='<div class="al" style="background:#EDFCF2;border:1px solid #B7E8C4;color:#1A9E47">\\u{1F4F1} WhatsApp connected \\u2014 send & receive tasks</div>';
...
if((s.pend+s.act)>0&&S.waOk)h+='<button class="bwa" onclick="sAll()">'+WI+' Send all pending to WhatsApp</button>';
h+='<div>';
if(!f.length)h+='<div class="empty"><div style="font-size:36px;margin-bottom:8px">'+(S.search?'\\u{1F50D}':'\\u2728')+'</div><div style="font-size:15px;font-weight:600">'+(S.search?'No matches':'No tasks yet')+'</div><div style="font-size:13px;margin-top:4px">Tap + or send a WhatsApp message</div></div>';
else f.forEach(t=>{
const p=P[t.priority]||P.medium,st=ST[t.status]||ST.pending,d=t.status==='done';
h+='<div class="tc'+(d?' dn':'')+'" style="border-left-color:'+p.c+'"><div class="tc-top"><button class="chk'+(d?' on':'')+'" onclick="tog(\\''+t.id+'\\')">'+(d?'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>':'')+'</button><div style="flex:1;min-width:0"><div class="tc-t'+(d?' dn':'')+'">'+esc(t.title)+'</div>'+(t.notes?'<div class="tc-n">'+esc(t.notes)+'</div>':'')+'<div class="tc-m"><button class="badge" style="background:'+st.bg+';color:'+st.c+'" onclick="cyc(\\''+t.id+'\\')">'+st.l+'</button>'+(t.due_date?'<span style="font-size:12px;font-weight:500;color:'+(isOD(t.due_date,t.status)?'#E8453C':isTd(t.due_date)?'#E8912C':'#9C968D')+'">\\u{1F4C5} '+fD(t.due_date)+(isOD(t.due_date,t.status)?' overdue':'')+'</span>':'')+(t.reminder_time&&!d?'<span style="font-size:11px;color:#3B82F6;font-weight:600">\\u{1F514} '+fT(t.reminder_time)+'</span>':'')+(t.source==='whatsapp'?'<span style="font-size:10px;color:#C4BFB7;font-weight:600">via WA</span>':'')+'</div></div></div>';
...
app.listen(PORT,()=>{console.log('\\n\\u{1F680} TaskFlow on port '+PORT);console.log('\\u{1F4F1} WhatsApp: '+(tw?'Connected':'Off'));console.log('\\u{1F4BE} Tasks: '+db.prepare('SELECT COUNT(*)as c FROM tasks').get().c+'\\n')});
